class Rock {
    cells: Set<{ x: number, y: number }> = new Set();

    constructor(paths: { x: number, y: number }[]) {
        let previous = paths[0];
        this.cells.add(previous);
        for (let i = 1; i < paths.length; i++) {
            let current = paths[i];
            if (current.x == previous.x) {
                if (current.y > previous.y) {
                    for (let y = previous.y + 1; y < current.y; y++) {
                        this.cells.add({ x: current.x, y: y });
                    }
                }
                else {
                    for (let y = previous.y - 1; y > current.y; y--) {
                ...
                if (current.x > previous.x) {
                    for (let x = previous.x + 1; x < current.x; x++) {
                        this.cells.add({ x: x, y: current.y });
                    }
                }
                else {
                    for (let x = previous.x - 1; x > current.x; x--) {
